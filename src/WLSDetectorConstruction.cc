...
// changing to scintillator tiles
  ...
  fHoleRadius       = fWLSfiberRY + 0.03*2*fWLSfiberRY;    //  fWLSfiberRY;                             //       no cladding   
...
//  if(GetFiberRadius() < GetHoleRadius())
  if(GetFiberRadius() <= GetHoleRadius())
  {
    ...
    fPhysiHole = new G4PVPlacement(0, G4ThreeVector(0.3 * cm, 0.0 * cm, 0.0 * cm), fLogicHole, "Hole", logicScintillator, false, 1);
  }
